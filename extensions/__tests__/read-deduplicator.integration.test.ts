...
import { createReadTracker, type TrackEntry } from "../../../dotagents/agent-hooks/read-deduplicator/src/core/read-tracker";
...
describe("read-deduplicator / decision chain", () => {
  ...
  test("re-read after file was modified → allowed", () => {
    ...
    updateStillInContext(t, [{ role: "user", content: "version 1 content is here" }]);
  ...
  test("full cycle: read → block on re-read → allow after truncation → re-track", () => {
    ...
    updateStillInContext(t, [{ role: "user", content: "doc content still here" }]);
    expect(shouldBlock(t, "/tmp/doc.md", "fp-doc")).toBe(true);

    // Turn 50: truncated from payload → re-read allowed
    updateStillInContext(t, [{ role: "user", content: "something else entirely" }]);
    expect(shouldBlock(t, "/tmp/doc.md", "fp-doc")).toBe(false);

    // Agent re-reads, tracker updated
    t.track("/tmp/doc.md", "fp-doc", 50, "doc content");
    updateStillInContext(t, [{ role: "user", content: "doc content reloaded" }]);
    expect(shouldBlock(t, "/tmp/doc.md", "fp-doc")).toBe(true);
  });
  ...
  test("multiple files tracked independently", () => {
    ...
    updateStillInContext(t, [{ role: "user", content: "content XXXX is here but YYYY is gone" }]);
