...
/**
 * Sessionless execution context — no ExtensionAPI dependency.
 * Used by the Turnlock integration and any caller that does not have
 * a Pi session. Replaces AsyncExecutionContext when pi events are not needed.
 */
export interface SessionlessContext {
	cwd: string;
	currentSessionId: string;
	parentSessionId?: string;
	currentModelProvider?: string;
...
/** Options for injecting Turnlock-specific paths into sessionless execution. */
export interface SessionlessPathOverrides {
	/** If set, the async run directory is created here instead of ASYNC_DIR/<id>. */
	asyncDir?: string;
	/** If set, the runner writes its result JSON here instead of RESULTS_DIR/<id>.json. */
	resultPath?: string;
	/** If set, passed to the child process alongside existing env. */
	turnlockEnv?: Record<string, string>;
}

/** Payload emitted when an async run starts. */
 ...
 */
interface SessionlessChainParams {
	chain: ChainStep[];
	task?: string;
	goal?: string;
	attachRoot?: ImportedAsyncRoot & { agent: string; outputName?: string; label?: string };
	...
	agents: AgentConfig[];
	ctx: SessionlessContext;
	onStarted?: (payload: AsyncStartedPayload) => void;
	availableModels?: AvailableModelInfo[];
	cwd?: string;
	maxOutput?: MaxOutputConfig;
	artifactsDir?: string;
	...
	configToolBudget?: ResolvedToolBudget;
	globalConcurrencyLimit?: number;
	/** Turnlock-specific path/env overrides. */
	pathOverrides?: SessionlessPathOverrides;
}

/**
 * Execute a chain asynchronously without a Pi session.
 * Uses SessionlessContext and an optional onStarted callback instead of
 * ExtensionAPI event emission. Used by the Turnlock integration.
 */
export function executeSessionlessChain(
	id: string,
	params: SessionlessChainParams,
	...
	const onStarted = params.onStarted;
	const resultMode = params.resultMode ?? "chain";
	const inheritedNestedRoute = resolveInheritedNestedRouteFromEnv();
	const nestedAddress = inheritedNestedRoute ? resolveNestedParentAddressFromEnv() : undefined;
	const asyncDir = params.pathOverrides?.asyncDir ?? (inheritedNestedRoute
		...
		spawnResult = spawnRunner(
			{
				id,
				steps,
				resultPath: params.pathOverrides?.resultPath ?? (inheritedNestedRoute ? nestedResultsPath(inheritedNestedRoute.rootRunId, id) : path.join(RESULTS_DIR, `${id}.json`)),
				cwd: runnerCwd,
				...
				...(params.pathOverrides?.turnlockEnv ? { turnlockEnv: params.pathOverrides.turnlockEnv } : {}),
			},
			id,
			runnerCwd,
		);
	...
	if (spawnResult.pid) {
		...
		const startedPayload: AsyncStartedPayload = {
...
export function executeAsyncChain(
	...
	return executeSessionlessChain(id, {
		chain: params.chain,
		task: params.task,
		goal: params.goal,
		attachRoot: params.attachRoot,
		...
		ctx: params.ctx,
		onStarted: (payload) => {
			const nestedRoute = params.nestedRoute ?? resolveInheritedNestedRouteFromEnv();
			const nestedAddress = nestedRoute ? resolveNestedParentAddressFromEnv() : undefined;
			if (nestedRoute && nestedAddress) {
				try {
					writeNestedEvent(nestedRoute, {
						...
						child: {
							...
							leafIntercomTarget: payload.agents?.[0] ? params.childIntercomTarget?.(payload.agents[0], 0) : undefined,
							intercomTarget: payload.agents?.[0] ? params.childIntercomTarget?.(payload.agents[0], 0) : undefined,
							ownerState: "live",
							mode: payload.mode,
							state: "running",
							agent: payload.agent,
							agents: payload.agents,
							chainStepCount: payload.chainStepCount,
							parallelGroups: payload.parallelGroups,
							...(payload.timeoutMs !== undefined ? { timeoutMs: payload.timeoutMs, deadlineAt: payload.deadlineAt } : {}),
			...
			params.ctx.pi.events.emit(SUBAGENT_ASYNC_STARTED_EVENT, payload);
		},
		availableModels: params.availableModels,
		cwd: params.cwd,
		...
		chainSkills: params.chainSkills,
		sessionFilesByFlatIndex: params.sessionFilesByFlatIndex,
		thinkingOverridesByFlatIndex: params.thinkingOverridesByFlatIndex,
		contextForAgent: params.contextForAgent,
 ...
 */
interface SessionlessSingleParams {
	agent: string;
	task?: string;
	goal?: string;
	agentConfig: AgentConfig;
...
/**
 * Execute a single agent asynchronously without a Pi session.
 * Uses SessionlessContext and an optional onStarted callback.
 */
export function executeSessionlessSingle(
	id: string,
	params: SessionlessSingleParams,
): AsyncExecutionResult {
	const {
		agent,
		agentConfig,
	...
	try {
		...
		const message = error instanceof Error ? error.message : String(error);
		return formatAsyncStartError("single", `Failed to start async single run '${id}': ${message}`);
	}

	if (spawnResult.error) {
		return formatAsyncStartError("single", `Failed to start async single run '${id}': ${spawnResult.error}`);
	}

	if (spawnResult.pid && onStarted) {
		onStarted({
			lifecycleArtifactVersion: SUBAGENT_LIFECYCLE_ARTIFACT_VERSION,
			id,
			pid: spawnResult.pid,
...
export function executeAsyncSingle(
	...
	return executeSessionlessSingle(id, {
		agent: params.agent,
		task: params.task,
		goal: params.goal,
		...
		onStarted: (payload) => {
				...
				try {
					writeNestedEvent(inheritedNestedRoute, {
						...
						child: {
							...
							pid: payload.pid,
							ownerIntercomTarget: process.env.PI_SUBAGENT_INTERCOM_SESSION_NAME,
							leafIntercomTarget: params.childIntercomTarget?.(params.agent, 0),
							intercomTarget: params.childIntercomTarget?.(params.agent, 0),
							ownerState: "live",
							mode: "single",
							state: "running",
							agent: params.agent,
			...
			params.ctx.pi.events.emit(SUBAGENT_ASYNC_STARTED_EVENT, {
				lifecycleArtifactVersion: payload.lifecycleArtifactVersion,
				id: payload.id,
				pid: payload.pid,
				...
				...(payload.timeoutMs !== undefined ? { timeoutMs: payload.timeoutMs, deadlineAt: payload.deadlineAt } : {}),
				...(payload.turnBudget ? { turnBudget: payload.turnBudget } : {}),
				nestedRoute: payload.nestedRoute,
			});
		},
		cwd: params.cwd,
		maxOutput: params.maxOutput,
		...
		acceptance: params.acceptance,
		timeoutMs: params.timeoutMs,
		absoluteDeadlineAt: params.absoluteDeadlineAt,
		turnBudget: params.turnBudget,
		toolBudget: params.toolBudget,
		configToolBudget: params.configToolBudget,
	});
