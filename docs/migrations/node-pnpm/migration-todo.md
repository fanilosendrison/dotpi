...
- [!] Map `permission-enforcer.e2e.test.ts` to a separate Node parity target with its single test identity, fourteen assertions, two real Pi sessions, an isolated Node subprocess, real state and real telemetry, and no normative change; local Node 22.19.0 passes while cross-platform evidence remains pending.
