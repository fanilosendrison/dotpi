...
- [!] Map `permission-enforcer.test.ts` to a separate Node parity target with the same four test identities, five assertions, Jiti cache isolation, real state and no normative change; local Node 22.19.0 passes while cross-platform evidence remains pending.
